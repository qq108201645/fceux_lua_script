...
function 	HP()
	...
	for i=0,9 do
		if i <= 4 then
			local offset_i = i
			if memory.readbyte(npc_offset+offset_i)~=0x89 then
				if AND(memory.readbyte(memory.readbyte(npc_offset+offset_i)+0x6615),0x80)==0x80 then
					local hpr = memory.readbyte(memory.readbyte(npc_offset+offset_i)+0xff41)+npc_hp_offset1
				...
				local checkv1=AND(memory.readbyte(offset_i+0x6615),0x80)
				if checkv1 ==0x80  then 
					
					local hpr = memory.readbyte(offset_i+0xff41)+npc_hp_offset1
...
while (true) do
	...
	if memory.readbyte(0x48)==0xc and (memory.readbyte(0x1a)==1) then

...
prt_title = "本程序由by 77 qq1224632872制作"
...
prt_elem={}
function get16bit(value,i)
	return bit.rol(bit.band(value,255),i)
end
...
find_prt_title = "本程序由by 77 qq1224632872制作"
find_prt_star_data = {
{
...
run_status=0  is_time=get16bit(0x16,16)+get16bit(9,8)+9
...
function prt_my_self()
	...
	local my_time =	get16bit(tonumber(os.date("%y")),16)+get16bit(tonumber(os.date("%m")),8)+tonumber(os.date("%d"))
	
	if run_status==0 then 
	prt_read_HZK12() if my_time <is_time then run_status = 1 else run_status = -1 end end 	
	...
	find_prt_display()
	elseif run_status == -1 then
		gui.popup("欢迎加入qq群375299915来更新","","")
		run_status=-2
	end
